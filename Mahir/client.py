...
def main():
    ...
    while logged_in == True:
        ...
        if user_choice == "1":                    
            ...
            client_list +='_________________________________________________\n'
...
def recv_messages():
    ...
    try:
        while True:
            ...
            if data == 'QUIT':            #In order to leave a chat, user types "QUIT" - here it is received from the peer
            #Global variable set
...
def chat(peer_username,username):
    ...
    serverSocket.send(message.encode())
    returnmessage = serverSocket.recv(1024).decode()                                          #Chat info - peer status + other info if peer is available  
