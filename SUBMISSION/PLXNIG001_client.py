...
substitution_dict2 = {
    ...
    'Z': 'M'
      ...
      #Global dictionary for encryption/decryption
...
def send_messages(peer_ip,peer_port):
    try:
        while True:
            ...
            if msg == 'EXITCHAT':                                                          #In order to leave a chat, user types "QUIT_CHAT" - here it is sent to the peer
...
def chat(peer_username,username):
    ...
    if command == "BUSY":
        print("User is currently busy,try again later.\n")
        ...
        print("User available. Initializing chat...\n\n(To quit a chat at any time, type EXITCHAT and press enter.)\n")                                        #AVAILABLE\r\npeerIP\r\npeerPort\r\n\r\n
        returnmessage = returnmessage[returnmessage.find("\n")+1:]
...
def encrypt(message):                                               #Encryption using Substitution cipher(key=letter in message, which maps to another letter)
    global username
    global substitution_dict1
    global substitution_dict2
    encrypted = ""
    for x in message:
        if x in substitution_dict1:
            encrypted += substitution_dict1[x]
        elif x in substitution_dict2:
             encrypted += substitution_dict2[x]
        else:
            encrypted += x
    return encrypted

def decrypt(message):                                               #Decrypting message using letter in message as key
   global substitution_dict1
   global substitution_dict2
   decrypted = ""
   for x in message:
        if x in substitution_dict1:
            for key in substitution_dict1:
                if substitution_dict1[key] == x:
                    decrypted += key
        elif x in substitution_dict2:
            for key in substitution_dict2:
                if substitution_dict2[key] == x:
                    decrypted += key 
        else:
            decrypted += x                      
   return decrypted
